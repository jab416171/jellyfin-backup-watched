...
def restore(dryrun=False):
    ...
    for item in data['items']:
        if item['Type'] == "Person":
            ...
            found_item = item_search(all_items, "Episode", name=item['Name'], seriesname=item['SeriesName'], seasonname=item['SeasonName'], imdbid=item.get('imdbid'), tmdbid=item.get('tmdbid'), tvdbid=item.get('tvdbid'))
        if found_item:
